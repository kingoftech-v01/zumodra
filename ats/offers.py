...
class OfferTemplate(TenantAwareModel):
    """
    Template for standard offer packages.

    Provides reusable offer structures for different roles
    ...
    # Bonus structure
    signing_bonus_range = models.CharField(
        max_length=50,
        blank=True,
        help_text=_('e.g., "5000-15000" or "10% of base"')
    )
    annual_bonus_target = models.CharField(
        max_length=50,
        blank=True,
        help_text=_('e.g., "10-15%" or "5000-10000"')
    )

    # Equity
    include_equity = models.BooleanField(default=False)
    equity_type = models.CharField(
        max_length=20,
        choices=[(e.value, e.name.replace('_', ' ').title()) for e in EquityType],
        blank=True
    )
    equity_range = models.CharField(
        max_length=100,
        blank=True,
        help_text=_('e.g., "0.01-0.05%" or "1000-5000 shares"')
    )
    equity_vesting_schedule = models.CharField(
        max_length=100,
        default='4 years with 1 year cliff'
    )

    # Benefits
    benefits_summary = models.TextField(blank=True)
    pto_days = models.PositiveIntegerField(default=15)
    health_benefits = models.BooleanField(default=True)
    retirement_contribution = models.CharField(
        max_length=50,
        blank=True,
        help_text=_('e.g., "4% match"')
    )

    # Offer letter template
    letter_template = models.TextField(
        blank=True,
        help_text=_('Django template for offer letter')
    )
    ...
    # Expiration
    default_expiration_days = models.PositiveIntegerField(default=7)
    ...
    class Meta:
        ...
        ordering = ['employment_type', 'experience_level', 'name']
        constraints = [
...
class OfferApproval(TenantAwareModel):
    """
    Individual approval record for an offer.
    """
    ...
    status = models.CharField(
        max_length=20,
        choices=[(s.value, s.name.replace('_', ' ').title()) for s in ApprovalStatus],
    ...
    class Meta:
        ...
        ordering = ['level__level', '-requested_at']

    def __str__(self):
        return f"Approval for {self.offer} by {self.approver}"
    ...
    def is_expired(self) -> bool:
        ...
        if not self.expires_at:
            return False
        return timezone.now() > self.expires_at and self.is_pending

    def approve(self, notes: str = ''):
        """Approve the offer at this level."""
    ...
    def reject(self, reason: str):
        """Reject the offer at this level."""
        ...
        if user is not None:
            ...
            if approval.approver and approval.approver != user:
                if not (getattr(user, 'is_superuser', False) or
                        (hasattr(user, 'has_perm') and user.has_perm('ats.approve_any_offer'))):
                    logger.warning(
