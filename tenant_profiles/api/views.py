...
from core.security.audit import AuditLogger, AuditAction, audit_data_access
...
from ..models import (
    ...
    Review, CandidateCV, StudentProfile, CoopTerm, FreelancerProfile
)
from .serializers import (
    ...
    FreelancerProfileSerializer, FreelancerProfileCreateSerializer,
    FreelancerProfileUpdateSerializer, FreelancerProfileListSerializer
)
...
class KYCVerificationViewSet(viewsets.ModelViewSet):
    ...
    def retrieve(self, request, *args, **kwargs):
        """Retrieve KYC verification with audit logging."""
        instance = self.get_object()

        # Log access to sensitive KYC document
        AuditLogger.log(
...
class EmploymentVerificationResponseView(APIView):
    """
...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
@audit_data_access('kyc_documents_list', lambda request: str(request.user.id))
...
class FreelancerProfileViewSet(viewsets.ModelViewSet):
    """
    ViewSet for freelancer profile management.

    Provides CRUD for freelancer profiles with public browsing capabilities.

    list: Browse freelancer profiles (public endpoint, filterable)
    retrieve: Get specific freelancer profile (public)
    create: Create own freelancer profile (authenticated)
    update: Update own freelancer profile (authenticated, owner only)
    partial_update: Partially update own freelancer profile
    destroy: Delete own freelancer profile (owner only)

    Custom actions:
    - me: Get, create, or update current user's freelancer profile
    - available: List only available freelancers
    - verified: List only verified freelancers
    """
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
    filter_backends = [DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter]
    filterset_fields = [
        'availability_status', 'is_verified', 'remote_only',
        'willing_to_relocate', 'country', 'hourly_rate_currency'
    ]
    search_fields = [
        'professional_title', 'bio', 'skills',
        'city', 'country', 'user__first_name', 'user__last_name'
    ]
    ordering_fields = [
        'created_at', 'hourly_rate', 'average_rating',
        'completed_projects', 'years_of_experience'
    ]
    ordering = ['-created_at']
    lookup_field = 'uuid'
    ...
    def get_queryset(self):
        ...
        queryset = FreelancerProfile.objects.select_related('user').prefetch_related('categories')

        # For list action, use lightweight filtering
        if self.action == 'list':
            return queryset.filter(is_verified=True)  # Only show verified profiles in public list
    ...
    def get_serializer_class(self):
        ...
        if self.action == 'list':
            return FreelancerProfileListSerializer
        elif self.action == 'create':
            return FreelancerProfileCreateSerializer
    ...
    def get_permissions(self):
        ...
        if self.action in ['list', 'retrieve', 'available', 'verified']:
            # Public read access for browsing
            return [permissions.AllowAny()]
        elif self.action in ['create', 'me']:
            # Authenticated users can create/view own profile
    ...
    def perform_update(self, serializer):
        """Update freelancer profile, check ownership."""
        if serializer.instance.user != self.request.user:
            from rest_framework.exceptions import PermissionDenied
            raise PermissionDenied("You can only update your own freelancer profile")
        serializer.save()

    def perform_destroy(self, instance):
        """Delete freelancer profile, check ownership."""
        if instance.user != self.request.user:
            from rest_framework.exceptions import PermissionDenied
            raise PermissionDenied("You can only delete your own freelancer profile")
        instance.delete()

    @action(detail=False, methods=['get', 'post', 'put', 'patch'])
    def me(self, request):
        """Get, create, or update current user's freelancer profile."""
        ...
        if request.method == 'GET':
            ...
            serializer = FreelancerProfileCreateSerializer(
                data=request.data,
                context={'request': request}
            )
            serializer.is_valid(raise_exception=True)
    ...
    def available(self, request):
        """List only available freelancers."""
        ...
        if page is not None:
            serializer = FreelancerProfileListSerializer(page, many=True, context={'request': request})
            return self.get_paginated_response(serializer.data)

        serializer = FreelancerProfileListSerializer(queryset, many=True, context={'request': request})
        return Response(serializer.data)

    @action(detail=False, methods=['get'])
    def verified(self, request):
        """List only verified freelancers."""
