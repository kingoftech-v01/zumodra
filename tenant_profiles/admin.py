...
class FreelancerProfileAdmin(admin.ModelAdmin):
    """Admin for FreelancerProfile model - Freelancer user profiles (NOT tenants)."""
    list_display = [
        'user_display', 'professional_title', 'availability_status_badge',
        'hourly_rate_display', 'rating_display', 'is_verified',
        'completed_projects', 'completed_services', 'created_at'
    ]
    list_filter = [
        'availability_status', 'is_verified', 'identity_verified',
        'payment_method_verified', 'remote_only', 'willing_to_relocate',
        'country', 'created_at'
    ]
    search_fields = [
        'user__email', 'user__first_name', 'user__last_name',
        'professional_title', 'bio', 'city', 'country', 'skills'
    ]
    readonly_fields = [
        'uuid', 'created_at', 'updated_at', 'last_active_at',
        'verification_date', 'completed_projects', 'completed_services',
        'total_earnings', 'average_rating', 'total_reviews'
    ]
    raw_id_fields = ['user', 'service_provider']
    ...
    fieldsets = (
        ...
        ('Professional Info', {
            'fields': ('professional_title', 'bio', 'years_of_experience')
        }),
        ('Availability', {
            'fields': (
        ...
        ('Platform Stats (Read-Only)', {
            ...
            'description': 'These statistics are automatically updated by the system.'
        }),
    ...
    def user_display(self, obj):
        ...
        user = obj.user
        return f"{user.email} ({user.get_full_name() or 'No name'})"
    ...
    def hourly_rate_display(self, obj):
        """Display formatted hourly rate with currency."""
        return f"{obj.hourly_rate_currency} {obj.hourly_rate}"
    hourly_rate_display.short_description = 'Hourly Rate'

    def rating_display(self, obj):
        """Display average rating with stars and color."""
        if obj.average_rating is None:
            return format_html('<span style="color: gray;">No reviews</span>')
        ...
        return format_html(
            '<span style="color: {};">{}</span> <span style="color: gray;">({:.1f} / {} reviews)</span>',
            color, stars, rating, obj.total_reviews
        )
