...
for i in {1..50}; do
  curl -H "Authorization: Bearer $TOKEN" \
    http://localhost:8002/api/v1/jobs/jobs/ | jq '.detail'
done
```
...
curl -i -H "Authorization: Bearer $TOKEN" \
  http://localhost:8002/api/v1/jobs/jobs/ | grep -i "x-ratelimit"
```
...
for i in {1..100}; do
  curl -s -H "Authorization: Bearer $ADMIN_TOKEN" \
    http://localhost:8002/api/v1/jobs/jobs/ | jq '.detail' || echo "Request $i"
done
```
