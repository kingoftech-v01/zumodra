...
class TestBackgroundCheckService:
    ...
    def test_initiate_check_success(self, tenant, application, user, mocker):
        ...
        mock_create_candidate = mocker.patch(
            'jobs.background_checks.CheckrProvider.create_candidate',
            return_value={'id': 'checkr_cand_123'}
        )
        mock_create_invitation = mocker.patch(
            'jobs.background_checks.CheckrProvider.create_invitation',
            return_value={'id': 'checkr_inv_456', 'status': 'pending'}
        )
        mock_create_report = mocker.patch(
            'jobs.background_checks.CheckrProvider.create_report',
    ...
    def test_get_report(self, tenant, application, user, mocker):
        ...
        mock_get_report = mocker.patch(
            'jobs.background_checks.CheckrProvider.get_report',
...
class TestBackgroundCheckAPI:
    ...
    def test_initiate_background_check_endpoint(self, api_client, tenant, application, user, mocker):
        """Test POST /api/v1/jobs/applications/{uuid}/background-check/initiate/"""
        api_client.force_authenticate(user=user)

        # Mock provider
        mocker.patch('jobs.background_checks.CheckrProvider.create_candidate', return_value={'id': 'cand_api'})
        mocker.patch('jobs.background_checks.CheckrProvider.create_invitation', return_value={'id': 'inv_api'})
        mocker.patch('jobs.background_checks.CheckrProvider.create_report', return_value={'id': 'report_api'})

        url = f'/api/v1/jobs/applications/{application.uuid}/background-check/initiate/'
        data = {
            'package': 'standard',
            'consent_given': True
        }
    ...
    def test_get_background_check_status(self, api_client, tenant, application, user):
        """Test GET /api/v1/jobs/applications/{uuid}/background-check/status/"""
        api_client.force_authenticate(user=user)
        ...
        url = f'/api/v1/jobs/applications/{application.uuid}/background-check/status/'
    ...
    def test_get_background_check_report(self, api_client, tenant, application, user, mocker):
        """Test GET /api/v1/jobs/applications/{uuid}/background-check/report/"""
        api_client.force_authenticate(user=user)
        ...
        mocker.patch(
            'jobs.background_checks.BackgroundCheckService.get_report',
            return_value={'status': 'complete', 'full': 'report'}
        )

        url = f'/api/v1/jobs/applications/{application.uuid}/background-check/report/'
        response = api_client.get(url)
...
class TestBackgroundCheckIntegration:
    ...
    def test_complete_background_check_workflow(self, tenant, application, user, mocker):
        ...
        mocker.patch('jobs.background_checks.CheckrProvider.create_candidate', return_value={'id': 'workflow_cand'})
        mocker.patch('jobs.background_checks.CheckrProvider.create_invitation', return_value={'id': 'workflow_inv'})
        mocker.patch('jobs.background_checks.CheckrProvider.create_report', return_value={'id': 'workflow_report'})
    ...
    def test_background_check_with_adverse_results(self, tenant, application, user, mocker):
        ...
        mocker.patch('jobs.background_checks.CheckrProvider.create_candidate', return_value={'id': 'adverse_cand'})
        mocker.patch('jobs.background_checks.CheckrProvider.create_invitation', return_value={'id': 'adverse_inv'})
        mocker.patch('jobs.background_checks.CheckrProvider.create_report', return_value={'id': 'adverse_report'})
