...
```django
<form hx-post="{% url 'frontend:jobs:job-create' %}"
      hx-target="#job-list"
      hx-swap="beforeend">
    {% csrf_token %}
    <!-- Form fields -->
...
<div hx-get="{% url 'frontend:jobs:job-list' %}?page={{ page_obj.next_page_number }}"
     hx-trigger="revealed"
     hx-swap="afterend">
    Loading more...
</div>
...
class JobPostingAPITests(APITestCase):
    ...
    def test_list_jobs(self):
        """Test listing jobs."""
        url = reverse('api:v1:jobs:job-list')
    ...
    def test_publish_job(self):
        ...
        url = reverse('api:v1:jobs:job-publish', args=[self.job.uuid])
...
## Testing
- Coverage: 85%
- Run tests: pytest apps/jobs/tests/
```
