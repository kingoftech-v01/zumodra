...
The Appointment app provides comprehensive scheduling and booking functionality for Zumodra's multi-tenant platform. It enables businesses to manage services, staff schedules, business hours, and customer appointments with conflict detection, automated notifications, and flexible rescheduling workflows.
...
Located in `appointment/models.py`:
...
#### Frontend Views (`appointment/views.py`)
...
#### Admin Views (`appointment/views_admin.py`)
...
#### Customer Views (`appointment/views_customer.py`)
...
#### API Views (`appointment/api/viewsets.py`)
...
#### Frontend URLs (`frontend:appointment:*`)

```python
# Main appointment interface
frontend:appointment:get_user_appointments
frontend:appointment:display_appointment (appointment_id)

# Booking
frontend:appointment:appointment_request (service_id)
frontend:appointment:appointment_request_submit

# Rescheduling
frontend:appointment:prepare_reschedule_appointment (id_request)
frontend:appointment:reschedule_appointment_submit
frontend:appointment:confirm_reschedule (id_request)

# Client flow
frontend:appointment:appointment_client_information (appointment_request_id, id_request)
frontend:appointment:enter_verification_code (appointment_request_id, id_request)
frontend:appointment:default_thank_you (appointment_id)

# Management
frontend:appointment:delete_appointment (appointment_id)
```

#### Customer URLs (`appointment_customer:*`)

```python
# Self-service portal
appointment_customer:my_appointments
appointment_customer:appointment_detail (appointment_id)
appointment_customer:cancel_appointment (appointment_id)
```

#### Legacy URLs (`appointment:*`)

The app includes legacy URL patterns for backward compatibility at `/appointment/`.

### Templates

Located in `templates/appointment/` (if exists) or integrated into admin templates:
...
**Customer Templates:**
- `appointment/customer_appointments.html` - Customer appointment list
- `appointment/booking_interface.html` - Booking flow interface
- `appointment/reschedule_interface.html` - Rescheduling interface
- `appointment/thank_you.html` - Confirmation page
...
When adding features to the Appointment app:
...
3. Create/update templates in `templates/appointment/`
...
APPOINTMENT_PAYMENT_URL=/payments/appointment/
APPOINTMENT_THANK_YOU_URL=/appointment/thank-you/
...
For questions or issues related to the Appointment app:
