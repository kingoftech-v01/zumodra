...
def sync_job_to_public(self, job_id: str, tenant_schema_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        catalog_data = {
            ...
            'category_names': [cat.name for cat in job.categories.all()] if hasattr(job, 'categories') else [],
            'category_slugs': [cat.slug for cat in job.categories.all()] if hasattr(job, 'categories') else [],
