...
from interviews.forms import PersonalInformationForm, ServiceForm, StaffDaysOffForm, StaffWorkingHoursForm
from interviews.messages_ import appt_updated_successfully
from interviews.settings import APPOINTMENT_PAYMENT_URL
from interviews.utils.date_time import (
    convert_12_hour_time_to_24_hour_time, convert_str_to_date, convert_str_to_time, get_ar_end_time)
from interviews.utils.db_helpers import (
    ...
    working_hours_exist)
from interviews.utils.email_ops import send_reset_link_to_staff_member
from interviews.utils.error_codes import ErrorCode
from interviews.utils.json_context import convert_appointment_to_json, get_generic_context, json_response
from interviews.utils.permissions import check_entity_ownership
from interviews.utils.session import handle_email_change
...
def prepare_user_profile_data(user, staff_user_id):
    ...
    if user.is_superuser and staff_user_id is None:
        ...
        btn_staff_me_link = reverse('interviews:make_superuser_staff_member')
        if StaffMember.objects.filter(user=user).exists():
            btn_staff_me = _("Remove me")
            btn_staff_me_link = reverse('interviews:remove_superuser_staff_member')
        data = {
            'error': False,
    ...
    if staff_user_id and staff_user_id != user.pk and not user.is_superuser:
        return {
            ...
            'extra_context': {'message': _("You can only view your own profile"),
                              'back_url': reverse('interviews:user_profile')},
            'status_code': 403
        }
    ...
    if not staff_member:
        return {
            'error': True,
            'extra_context': {'message': _("Not authorized."), 'back_url': reverse('interviews:user_profile')},
            'status_code': 403
        }
...
def handle_day_off_form(day_off_form, staff_member):
    ...
    if day_off_form.is_valid():
        ...
        day_off.save()
        redirect_url = reverse('interviews:user_profile',
...
def handle_working_hours_form(staff_member, day_of_week, start_time, end_time, add, wh_id=None):
    ...
    redirect_url = reverse('interviews:user_profile', kwargs={'staff_user_id': staff_member.user.id}) \
        if staff_member.user.id else reverse('interviews:user_profile')
