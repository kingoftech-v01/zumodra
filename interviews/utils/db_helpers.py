# db_helpers.py
# Path: interviews/utils/db_helpers.py
...
from interviews.settings import (
...
from interviews.utils.date_time import combine_date_and_time, get_weekday_num
...
Appointment = apps.get_model('interviews', 'Appointment')
AppointmentRequest = apps.get_model('interviews', 'AppointmentRequest')
WorkingHours = apps.get_model('interviews', 'WorkingHours')
DayOff = apps.get_model('interviews', 'DayOff')
PaymentInfo = apps.get_model('interviews', 'PaymentInfo')
StaffMember = apps.get_model('interviews', 'StaffMember')
Config = apps.get_model('interviews', 'Config')
Service = apps.get_model('interviews', 'Service')
EmailVerificationCode = apps.get_model('interviews', 'EmailVerificationCode')
AppointmentRescheduleHistory = apps.get_model('interviews', 'AppointmentRescheduleHistory')
...
def schedule_email_reminder(appointment, request, appointment_datetime=None):
    ...
    relative_reschedule_url = reverse('interviews:prepare_reschedule_appointment', args=[ar_id_request])
...
def get_appointment_buffer_time():
    ...
    """
    from interviews.models import Config
...
def get_working_hours_for_staff_and_day(staff_member, day_of_week):
    ...
    # If no working hours are set for that day, staff member is not working
    # We don't assume default working hours to avoid implying they work every day
