# email_ops.py
# Path: interviews/utils/email_ops.py
...
from interviews import messages_ as email_messages
from interviews.email_sender import notify_admin, send_email
from interviews.logger_config import get_logger
from interviews.models import AppointmentRequest, EmailVerificationCode, PasswordResetToken, Appointment
from interviews.settings import APPOINTMENT_PAYMENT_URL
from interviews.utils.date_time import convert_24_hour_time_to_12_hour_time
from interviews.utils.db_helpers import get_absolute_url_, get_website_name
from interviews.utils.ics_utils import generate_ics_file
...
def send_thank_you_email(ar: AppointmentRequest, user, request, email: str, appointment_details=None,
    ...
    if account_details:
        ...
        relative_set_passwd_link = reverse('interviews:set_passwd', args=[ui_db64, token.token])
        set_passwd_link = get_absolute_url_(relative_set_passwd_link, request=request)

    relative_reschedule_url = reverse('interviews:prepare_reschedule_appointment', args=[ar.get_id_request()])
...
def send_reschedule_confirmation_email(request, reschedule_history, appointment_request, first_name: str, email: str):
    ...
    relative_confirmation_link = reverse('interviews:confirm_reschedule', args=[reschedule_history.id_request])
